...
const Fundstatus = async ({
  ...
  const effectiveYear = resolved.year ?? currentFY;
  ...
  if (resolved.nit) {
    try {
      parsedNit = Number.parseInt(resolved.nit);
  ...
  const works = await db.worksDetail.findMany({
    where: {
      ...
      ...(resolved.fundType && {
        ApprovedActionPlanDetails: { schemeName: resolved.fundType },
      }),
      tenderStatus: { in: ["AOC"] },
    },
    include: {
    ...
    <div className="p-4 space-y-6">
      <Suspense fallback={<FilterSkeleton />}>
        <CombinedFilter
          ...
          fundTypes={fundTypes}
          selectedNit={resolved.nit}
          selectedFundType={resolved.fundType}
